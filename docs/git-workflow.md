...
**Le scope s'écrit en dur dans `commit-message.prefix`.** Le nom de l'option trompe : Dependabot construit d'abord le titre de la pull request, dont le sujet du commit n'est qu'une copie tronquée. `commit-message.prefix` gouverne donc les deux. L'option `include: "scope"` de Dependabot ajouterait automatiquement `(deps)` ou `(deps-dev)` selon le type de dépendance — or `deps-dev` n'appartient pas à la liste fermée du [§3](#3-convention-de-commits), et `pr-title.yml` rejetterait toute pull request touchant une dépendance de développement, c'est-à-dire la majorité. En posant `chore(deps)` en entier, le scope est garanti.

**La casse, elle, ne l'est pas.** Le service hébergé capitalise la description qu'il compose — `chore(deps): Bump alembic from 1.19.1 to 1.19.2` — et n'offre aucune option pour l'en empêcher, quelle que soit la logique lisible dans les sources de `dependabot-core`. `pr-title.yml` relâche donc la règle de minuscule pour les seules pull requests dont l'auteur est `dependabot[bot]`, en la maintenant pour tout le monde d'autre. Assouplir le motif pour tous aurait perdu une règle choisie délibérément ; exempter le bot du job entier aurait aussi perdu la vérification du scope, seul garde-fou si quelqu'un remanie `dependabot.yml` plus tard. La contrepartie est que `main` porte des sujets capitalisés sur les commits `chore(deps)`, sans effet sur les releases puisque `chore` n'en produit aucune.
