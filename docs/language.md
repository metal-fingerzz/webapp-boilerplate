...
- **C'était presque déjà le cas.** Au moment d'écrire cette règle, le français hors Markdown tenait dans deux fichiers de workflow, que la pull request qui l'a formalisée a traduits dans le même mouvement ; tout le reste — commentaires Python, `alembic.ini`, `.gitignore`, `pyproject.toml` — était en anglais, hérité des templates Alembic et Vite. La migration s'est arrêtée à ces deux fichiers.
- **La règle a dérivé une fois.** Dix jours plus tard, `.github/dependabot.yml` est arrivé commenté en français, et la revue de sa pull request n'a pas relevé l'écart ; il a fallu quatre jours de plus et un changement sans rapport, qui rouvrait le fichier, pour que la traduction se fasse. La règle tient donc par la relecture, sans contrôle automatique — un choix assumé tant que le dépôt se relit entièrement.
